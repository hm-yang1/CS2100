...
const char *student_number = "A0287336U";
const char *student_name = "Yang Hanming";
const char *student_tut_grp = "10";
...
int twos_to_ones(int x)
{
    if (x < 0) 
    {
        //if negative, set to positive then flip the bits
        int tmp = -x;
        return ~tmp;
    }
    return x;
...
{
    for (int j = n-1; j >= 0; j--) {
        s[j] = (i & 1) + '0'; // Get right most bit as char
        i >>= 1; //Shift right
    }
    s[n] = '\0';
}
 ...
 * 
 * WAIT do I convert the binary to decimal int or 2's complement int?
 *
...
{
    //isPositive == 1 means n > 0;
    int isPositive = s[0] == '1' ? 0 : 1;
    int multiplier = 1;
    int result = 0;
...
int perform_addition(int n, char *x, char *y, char *z, char *c)
{
    ...
    if (validate_1s_complement(op1, n) != 0 || validate_1s_complement(op2, n) != 0 || validate_1s_complement(sum, n) != 0) 
    {
        return -1;
    }

    //Get sum in binary and the carry string
    int carryIn = 0;
    for (int i = n - 1; i >= 0; i--)
    {
        int a = x[i] - '0';
